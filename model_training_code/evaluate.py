...
def load_model(configuration, device):
    ...
    config = AutoConfig.from_pretrained(
        model_name_or_path, cache_dir=configuration["cache_dir"]
    )
...
def report(configuration, y_true, y_pred, label_names, labels=None):
    ...
    if (configuration["test_corpora"] is not None) and (__name__ == "__main__"):
        file_name = (
            configuration["output_dir"].split("/")[-1]
            + "-evaluated-on-"
...
if __name__ == "__main__":
    ...
    test_dataset = test_dataset.map(
        speech_file_to_array_fn, fn_kwargs=dict(processor=processor)
    )
    ...
    print(
        classification_report(y_true, y_pred, labels=labels, target_names=label_names)
    )
